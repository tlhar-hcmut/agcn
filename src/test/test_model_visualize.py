...
class TestModelVisualize(BaseTestCase):
    ...
    def test_tk_temporal(self):
        model = M.TKNet(num_class=12, stream=[
                        1], input_size=cfg_train.input_size, cls_graph=NtuGraph)
        self.summary_to_file(title="Temporal", model=model.to('cuda'), input_data=torch.zeros(
            (1, *cfg_train.input_size)), depth=15, col_width=20, col_names=["input_size", "kernel_size", "output_size", "num_params"])
