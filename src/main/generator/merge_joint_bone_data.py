...
def gen_joint_motion(dataset, phase):
    ...
    """
    print(dataset, phase)
    file_joint = default_arg["path_data_preprocess"] + "/%s/%s/%s_joint.npy" % (
        dataset,
        phase,
        phase,
    )
    file_bone = default_arg["path_data_preprocess"] + "/%s/%s/%s_bone.npy" % (
        dataset,
        phase,
        phase,
    )
    file_joint_bone = default_arg[
        "path_data_preprocess"
    ] + "/%s/%s/%s_joint_bone.npy" % (dataset, phase, phase)
