...
xfile.mkdir("output_train_not_sm_thucth_xsub")
xfile.mkdir("output_train_not_sm_thucth_xsub/predictions")
xfile.mkdir("output_train_not_sm_thucth_xsub/loss")
xfile.mkdir("output_train_not_sm_thucth_xsub/confusion_matrix")
...
class TrainXSub:
    def __init__(self, pretrained_path=None):
        ...
        self.logger = {
            "val": setup_logger(name="val_logger",
                                log_file="output_train_not_sm_thucth_xsub/eval_val.log",
                                level=logging.DEBUG),
            "train": setup_logger(name="train_logger",
                                  log_file="output_train_not_sm_thucth_xsub/eval_train.log",
                                  level=logging.DEBUG),
            "val_confusion": setup_logger(name="train_confusion_logger",
                                          log_file="output_train_not_sm_thucth_xsub/confusion_val.log",
                                          level=logging.DEBUG),
            "train_confusion": setup_logger(name="train_confusion_logger",
                                            log_file="output_train_not_sm_thucth_xsub/confusion_train.log",
                                            level=logging.DEBUG),
    ...
    def __draw_confusion_matrix(self, epoch=-1, full_predictions=None, loader_name='val'):
        ...
        plot_confusion_matrix(
            df_confusion, file_name="output_train_not_sm_thucth_xsub/confusion_matrix/cf_mat_{}_{}.png".format(loader_name, epoch), title="confution matrix "+loader_name)
            ...
            if accuracy > self.best_acc[ln]["value"]:
                ...
                if save_score:
                    with open('{}/epoch{}_{}_predict_vector.pkl'.format(
                            "output_train_not_sm_thucth_xsub/predictions", epoch, ln), 'wb') as f:
